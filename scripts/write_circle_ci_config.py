#!/usr/bin/env python3
"""Write a short circle bifurcation-diagram config for local CI."""
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--base-config",
        default="json_project_files/circle_test.json",
        help="Base circle JSON file.",
    )
    parser.add_argument("--steps", type=int, default=32)
    parser.add_argument("--step-size", type=float, default=0.02)
    parser.add_argument("--max-step-size", type=float, default=0.04)
    parser.add_argument("--deflation-attempts", type=int, default=5)
    parser.add_argument("--continuation-fail-attempts", type=int, default=8)
    parser.add_argument(
        "--knots",
        type=float,
        nargs="+",
        default=[-0.99, -0.7, -0.2, 0.2, 0.7, 0.99],
    )
    parser.add_argument(
        "--clean-project",
        action="store_true",

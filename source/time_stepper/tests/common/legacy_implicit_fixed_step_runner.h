#ifndef TIME_STEPPER_TESTS_COMMON_LEGACY_IMPLICIT_FIXED_STEP_RUNNER_H
#define TIME_STEPPER_TESTS_COMMON_LEGACY_IMPLICIT_FIXED_STEP_RUNNER_H
...
struct direct_linear_solver
{
    template<class LinearOperator, class Vector>
    bool solve(const LinearOperator& linear_operator, const Vector& right_hand_side, Vector& solution)
    {
        return linear_operator.solve(right_hand_side, solution);
    }
};

template<std::size_t Dimension, class VectorOperations, class Problem, class LinearOperator, class Log>
std::array<typename VectorOperations::scalar_type, Dimension> integrate_legacy_implicit_fixed(
    ...
    const std::size_t step_count)
{
    ...
    using step_type = implicit_time_step<
        VectorOperations,
        Problem,
        LinearOperator,
    ...
    adaptation_type adaptation(
        ...
        step_size,
        &positivity);
    direct_linear_solver linear_solver;
    step_type step(
        &vector_operations,
        &adaptation,
        &log,
        ...
        parameter,
        method);
    step.set_newton_method(scalar_type(1e-13), 20);

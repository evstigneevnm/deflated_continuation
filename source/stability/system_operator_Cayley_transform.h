#ifndef __STABILITY__SYSTEM_OPERATOR_Cayley_transform_H__
#define __STABILITY__SYSTEM_OPERATOR_Cayley_transform_H__
...
class STABILITY_LEGACY_API(
    "system_operator_Cayley_transform is legacy; use matrix-free "
    "spectral transformations") system_operator_Cayley_transform
{
public:
    typedef typename VectorOperations::scalar_type  T;
    typedef typename VectorOperations::vector_type  T_vec;

    system_operator_Cayley_transform(VectorOperations* vec_ops_, NonlinearOperator* nonlin_op_, LinearOperator* lin_op_, LinearSystemSolver* lin_solv_, Log* log_):
    vec_ops(vec_ops_),
    ...
    ~system_operator_Cayley_transform()
    {
        vec_ops->stop_use_vector(r_v); vec_ops->free_vector(r_v);
    }

    void set_sigma_and_mu(const T sigma_, const T mu_)
    {
        sigma = sigma_;
        mu = mu_;
    }
    ...
    {
        // if(!linearization_set) throw std::runtime_error("system_operator_Cayley_transform: linearization point not set");
        ...
        while((!linear_system_converged)&&(iter<max_retries))
        {
            ...
            linear_system_converged = lin_solv->solve((*lin_op), r_v, v_out);   
...
private:
    ...
    void chech_nan_norm(const T_vec v_, const std::string& msg_)
    {
        ...
        if(std::isnan(norm_) )
        {
            throw std::runtime_error("system_operator_Cayley_transform:: chech_nan_norm: " + msg_);
        }
    }

...
std::size_t krylov_dimension(int argc, char** argv)
{
    if(argc > 4)
        return static_cast<std::size_t>(std::stoul(argv[4]));
    return 14;
}

std::size_t restart_dimension(int argc, char** argv)
{
    if(argc > 5)
        return static_cast<std::size_t>(std::stoul(argv[5]));
    return 4;
}

std::size_t desired_eigenvalues(int argc, char** argv)
{
    if(argc > 6)
        return static_cast<std::size_t>(std::stoul(argv[6]));
    return 2;
}
...
int main(int argc, char** argv)
{
    try
    {
        ...
        const auto factorization_started =
            std::chrono::steady_clock::now();
        std::vector<std::shared_ptr<factor_solver_type>> factor_solvers;
        factor_solvers.reserve(factors.size());
        ...
        const auto factorization_finished =
            std::chrono::steady_clock::now();
        ...
        options.desired_eigenvalues = desired_eigenvalues(argc, argv);
        options.krylov_dimension = krylov_dimension(argc, argv);
        options.restart_dimension = restart_dimension(argc, argv);
        ...
        const auto transformed_result = eigensolver.execute(
            ...
            options,
            &transformed_vectors);
        const auto solve_finished =
            std::chrono::steady_clock::now();

        const double factorization_seconds =
            std::chrono::duration<double>(
                factorization_finished - factorization_started).count();
        const double solve_seconds =
            std::chrono::duration<double>(
                solve_finished - solve_started).count();
        std::cout
            << std::setprecision(17)
            << "case=S80PI_n1"
            << " mode=exact_factorized_euler_shift_inverse"
            ...
            ++index)
        {
            std::cout
                ...
                << (complex_type(1.0, 0.0) -
                    factors[index].diagonal_shift).real()
                << " root_imag="
                << (complex_type(1.0, 0.0) -
                    factors[index].diagonal_shift).imag()
                << " rcond="
                << factor_solvers[index]->
                    reciprocal_condition_estimate()
                << " nnz=" << factor_solvers[index]->nonzeros()
                << '\n';
        }
        for(std::size_t index = 0;
        ...
        {
            throw std::runtime_error(
                "exact transformed eigensolver failed structurally: " +
        ...
        std::cout
            ...
            << " expected_across_angle_scan=4"
            << '\n';

        product_space->stop_use_vector(initial);
        product_space->free_vector(initial);
        std::cout
            << "S80PI_n1 exact transformed characterization: COMPLETED\n";
        return EXIT_SUCCESS;
    }
    catch(const std::exception& error)
    {
        std::cerr
            << "S80PI_n1 exact transformed characterization: FAILED: "
            << error.what() << '\n';
        return EXIT_FAILURE;
    }
}

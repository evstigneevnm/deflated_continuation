#ifndef __DISCRETIZATION_FOURIER_CODECS_FULL_REAL_FIELD_H__
#define __DISCRETIZATION_FOURIER_CODECS_FULL_REAL_FIELD_H__
...
public:
    ...
    full_real_field_2d(vector_operations_type* vector_operations, const r2c_index_space_2d& index_space):
        ...
        index_space_(index_space)
    {
        static_assert(std::is_same<typename complex_traits::real_type, scalar_type>::value,
            "full_real_field_2d scalar and complex value types must match");
        if(vector_operations_ == nullptr)
        {
            throw std::invalid_argument("full_real_field_2d requires vector operations");
        }
        if(vector_operations_->get_default_size() != state_size())
        {
            throw std::invalid_argument("full_real_field_2d vector size does not match the mean-zero real field");
        }
    ...
    {
        return index_space_.full_mean_zero_state_size();
    }
    ...
    void unpack(const vector_type& state, SpectralField& spectrum) const
    {
        ...
        for_each(
            [=] __DEVICE_TAG__ (const ordinal_type descriptor_index)
            {
                ...
                if(descriptor.kind == detail::coefficient_kind_2d::self_real)
                {
                    output[descriptor.spectrum_index] =
                        complex_traits::make(input[descriptor.state_offset], scalar_type(0));
                    return;
                }
    ...
    void pack(const SpectralField& spectrum, vector_type& state) const
    {
        ...
        for_each(
            [=] __DEVICE_TAG__ (const ordinal_type descriptor_index)
            {
                ...
                if(descriptor.kind == detail::coefficient_kind_2d::complex)
                {
    ...
    void pack_adjoint(const vector_type& state, SpectralField& spectrum) const
    {
        ...
        for_each(
            [=] __DEVICE_TAG__ (const ordinal_type descriptor_index)
            {
                ...
                if(descriptor.kind == detail::coefficient_kind_2d::self_real)
                {
                    output[descriptor.spectrum_index] = complex_traits::make(
                        input[descriptor.state_offset], scalar_type(0));
                    return;
                }
...
private:
    void initialize_descriptors()
    {
        ...
        append_boundary(host, 0, true, offset);
        ...
        append_boundary(host, index_space_.ny()/2, false, offset);

        if(static_cast<std::size_t>(offset) != state_size())
        {
            throw std::logic_error("full_real_field_2d descriptor count is inconsistent");
        }
    ...
    )
    {
        ...
        {
            host.push_back(descriptor_type{
                static_cast<std::ptrdiff_t>(index_space_.flat_index(0, iy)),
                -1,
                offset++,
                detail::coefficient_kind_2d::self_real
            });
        }
        ...
        host.push_back(descriptor_type{
            static_cast<std::ptrdiff_t>(index_space_.flat_index(index_space_.nx()/2, iy)),
            -1,
            offset++,
            detail::coefficient_kind_2d::self_real
        });
    ...
    void require_sizes(const vector_type& state, const SpectralField& spectrum) const
    {
        ...
        {
            throw std::invalid_argument("full_real_field_2d pack/unpack size mismatch");
        }
    }

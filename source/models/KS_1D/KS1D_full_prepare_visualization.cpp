...
int main(int argc, char const* argv[])
{
    ...
    std::string path_to_config_file = "json_project_files/KS1D_test_full.json";
    ...
    try
    {
        for(int argi = 1; argi < argc; ++argi)
        {
            ...
            if(path_to_config_file == "json_project_files/KS1D_test_full.json")
            {
        ...
        {
            throw std::runtime_error("Full KS1D visualization expects nonlinear_operator.discrete_problem_dimensions[0]");
        }
        const std::size_t physical_size = parameters.nonlinear_operator.N_size.at(0);
        if(physical_size < 4 || physical_size%2 != 0)
        {
            throw std::runtime_error("Full KS1D visualization expects an even physical size >= 4");
        }
        ...
        vec_ops_real state_vec_ops(state_size);

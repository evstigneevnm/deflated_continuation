...
#component tests
test_butcher_tableau_contract.bin: source/time_stepper/tests/test_butcher_tableau_contract.cpp $(TIME_STEPPER_TABLEAU_HEADERS)
	$(G++) $(G++FLAGS) $(IPROJECT) source/time_stepper/tests/test_butcher_tableau_contract.cpp -o $(BUILD_DIR)/test_butcher_tableau_contract.bin 2>$(RESULTS)

test_semidiscrete_problem_contract.bin: source/time_stepper/tests/test_semidiscrete_problem_contract.cpp $(TIME_STEPPER_SEMIDISCRETE_HEADERS)
	$(G++) $(G++FLAGS) $(IPROJECT) source/time_stepper/tests/test_semidiscrete_problem_contract.cpp -o $(BUILD_DIR)/test_semidiscrete_problem_contract.bin 2>$(RESULTS)
...
test_dae_analytical_contract.bin: source/time_stepper/tests/test_dae_analytical_contract.cpp $(TIME_STEPPER_TEST_COMMON_HEADERS) $(TIME_STEPPER_SEMIDISCRETE_HEADERS) $(SCFD_SERIAL_VECTOR_OPS_HEADERS)
	$(G++) $(G++FLAGS) $(IPROJECT) source/time_stepper/tests/test_dae_analytical_contract.cpp $(OPENMP) -o $(BUILD_DIR)/test_dae_analytical_contract.bin 2>$(RESULTS)

test_timestepper_nonlinear_regressions.bin: source/time_stepper/tests/test_nonlinear_system_regressions.cpp source/time_stepper/tests/common/nonlinear_benchmark_problems.h $(TIME_STEPPER_TEST_COMMON_HEADERS) $(TIME_STEPPER_LEGACY_IMPLICIT_HEADERS) $(SCFD_SERIAL_VECTOR_OPS_HEADERS)
	$(G++) $(G++FLAGS) $(IPROJECT) source/time_stepper/tests/test_nonlinear_system_regressions.cpp $(OPENMP) -o $(BUILD_DIR)/test_timestepper_nonlinear_regressions.bin 2>$(RESULTS)

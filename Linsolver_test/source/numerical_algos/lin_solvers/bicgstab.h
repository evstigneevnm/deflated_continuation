...
public:
    bicgstab(const vector_operations_type *vec_ops, 
        ...
        nonprecond_ri(bufs[6]), use_precond_resid_(true),
    ...
    {
        use_precond_resid_ = use_precond_resid;
    }
                          ...
                          vector_type &x)const
    {
        ...
        if (!use_precond_resid_) vec_ops_->assign(ri, nonprecond_ri);
        ...
        while (!monitor_.check_finished(x, *checked_ri))
        {
            ...
            if ((resid_recalc_freq_ == 0)||((monitor_.iters_performed()+1) % resid_recalc_freq_ != 0)) {
                if (!use_precond_resid_) {
                ...
                //ri := P*b - P*A*x0;
                A.apply(x, ri);                                                //ri := A*x
                vec_ops_->add_mul(T(1), b, -T(1), ri);                         //ri := -ri + b = -A*x + b
                if (!use_precond_resid_) vec_ops_->assign(ri, nonprecond_ri);
                if (prec_ != NULL) prec_->apply(ri);                           //ri := P*ri = -P*A*x + P*b
            }

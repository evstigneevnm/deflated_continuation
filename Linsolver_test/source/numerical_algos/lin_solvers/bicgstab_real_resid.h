...
public:
                          ...
                          vector_type &x)const
    {
        ...
        while (!monitor_.check_finished(x, *checked_ri))
        {
            ...
            if (std::isnan(beta)) { this->info("solve: stop iterations because beta is ind"); not_valid_coeff_faced = true; break; }
            if (std::isinf(beta)) { this->info("solve: stop iterations because beta is inf"); not_valid_coeff_faced = true; break; }
            ...
            if (std::isnan(alpha)) { this->info("solve: stop iterations because alpha is nan"); not_valid_coeff_faced = true; break; }
            if (std::isinf(alpha)) { this->info("solve: stop iterations because alpha is inf"); not_valid_coeff_faced = true; break; }
            ...
            if (std::isnan(omega_i)) { this->info("solve: stop iterations because omega_i is nan"); not_valid_coeff_faced = true; break; }
            if (std::isinf(omega_i)) { this->info("solve: stop iterations because omega_i is inf"); not_valid_coeff_faced = true; break; }

...
private:
    ...
    struct LinearOperator_SM
    {
        ...
        mutable scalar_type alpha, beta, v;
        ...
        LinearOperator_SM(const vector_operations_type *vector_operations_)
        {
            ...
            //z=(vector_type)malloc((vector_operations->sz_)*sizeof(scalar_type));
            vector_operations->init_vector(z);
            vector_operations->start_use_vector(z);

        }

        ~LinearOperator_SM()
        {
            vector_operations->stop_use_vector(z);
            vector_operations->free_vector(z);
        }
        ...
        void apply(const vector_type& u, vector_type& f) const
        {
            ...
            vector_operations->add_mul(-cTu_over_alpha, d, scalar_type(1.0), f);
        }
    ...
    struct Preconditioner_SM
    {
        ...
        {
            vector_operations = vector_operations_;
            //g=(vector_type)malloc((vector_operations->sz_)*sizeof(scalar_type));
            vector_operations->init_vector(g);
            vector_operations->start_use_vector(g);

        }
        ~Preconditioner_SM()
        {
            //free(g);
            vector_operations->stop_use_vector(g);
        ...
        {
            // //calc: y := mul_x*x
            vector_operations->assign_mul(-1.0/alpha, d, g);
            inherited_preconditioner->apply(x);
            inherited_preconditioner->apply(g);
            scalar_type dot_prod_num = vector_operations->scalar_prod(c, x);
            scalar_type dot_prod_din = vector_operations->scalar_prod(c, g);
            scalar_type mul_g=-dot_prod_num/(1.0+dot_prod_din);
            //calc: y := mul_x*x + mul_y*y
            vector_operations->add_mul(mul_g, g, scalar_type(1.0), x);
...
public:
    ...
    sherman_morrison_linear_system_solve(const Preconditioner *prec_, const VectorOperations *vec_ops_, Log *log_): linear_solver(vec_ops_,log_), prec(vec_ops_), oper(vec_ops_) 
    {

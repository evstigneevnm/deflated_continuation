...
public:
    ...
    void init_vector(vector_type& x, const std::size_t = 0) const
    {
        x = vector_type();
    }
    ...
    void start_use_vector(vector_type& x, const std::size_t sz_p = 0) const
    {
        if(x.is_free())
        {
    ...
    void start_use_multivector(multivector_type& x, std::size_t m) const
    {
        for(std::size_t i = 0; i < m; ++i)
        {
            start_use_vector(x[i]);
        }
    }

    void stop_use_multivector(multivector_type&, std::size_t) const
    {
    }

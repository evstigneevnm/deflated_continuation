...
// All vectors MUST contain size, hence vector_operations don't store size
// explicitly
...
struct cpu_vector_operations
{
    ...
    cpu_vector_operations( size_t sz_p, int use_high_precision_dot_product_ = 0, int use_threaded_dot_ = 0 )
        : sz_default_( sz_p ), use_threaded_dot( use_threaded_dot_ )
    {
        location = false;
        dot      = new dot_product<T, vector_type>( use_high_precision_dot_product_ );
        threaded_dot =
            new threaded_reduction<scalar_type, vector_type>( use_threaded_dot_, use_high_precision_dot_product_ );
    }
    ...
    void
    assign_mul( scalar_type mul_x, const vector_type &x, scalar_type mul_y, const vector_type &y, vector_type &z ) const
    {
    ...
    void add_mul(
        const scalar_type mul_x, const vector_type &x, const scalar_type mul_y, const vector_type &y,
        const scalar_type mul_z, vector_type &z
    ) const
    {
        if ( ( x.size() != y.size() ) && ( x.size() != z.size() ) )
        {
    ...
    void make_abs_copy( const vector_type &x, vector_type &y ) const
    {
        if ( x.size() != y.size() )
        {
            throw std::logic_error( "cpu_vector_operations::make_abs_copy: incorrect "
                                    "vector sizes provided" );
        }
    ...
    void max_pointwise( const scalar_type sc, const vector_type &x, vector_type &y ) const
    {
        if ( x.size() != y.size() )
        {
            throw std::logic_error( "cpu_vector_operations::max_pointwise: incorrect "
                                    "vector sizes provided" );
        }
        for ( size_t j = 0; j < x.size(); j++ )
        {
    ...
    void min_pointwise( const scalar_type sc, const vector_type &x, vector_type &y ) const
    {
        if ( x.size() != y.size() )
        {
            throw std::logic_error( "cpu_vector_operations::min_pointwise: incorrect "
                                    "vector sizes provided" );
        }
        for ( size_t j = 0; j < x.size(); j++ )
        {
    ...
    void mul_pointwise( vector_type &x, const scalar_type mul_y, const vector_type &y ) const
    {
        if ( x.size() != y.size() )
        {
            throw std::logic_error( "cpu_vector_operations::mul_pointwise: incorrect "
                                    "vector sizes provided" );
        }
        for ( size_t j = 0; j < x.size(); j++ )
        {
    ...
    ) const
    {
        if ( ( x.size() != y.size() ) && ( x.size() != z.size() ) )
        {
            throw std::logic_error( "cpu_vector_operations::div_pointwise: incorrect "
                                    "vector sizes provided" );
        }
        for ( size_t j = 0; j < x.size(); j++ )
        {
    ...
    assign_slices( const vector_type &x, const std::vector<std::pair<size_t, size_t>> slices, vector_type &y ) const
    {
        ...
        if ( sz_l < y.size() )
        {
            throw std::logic_error( "cpu_vector_operations::assign_slice: can only be "
                                    "applied to vectors of sizes x.size<=y.size" );
        }
        size_t index_y = 0;
        for ( auto &slice : slices )
        {
            ...
            if ( end > sz_l )
            {
                throw std::logic_error( "cpu_vector_operations::assign_slice: provided "
                                        "slice size is greater than input vector size." );
            }
    ...
    ) const
    {
        ...
        if ( sz_l < y.size() )
        {
            throw std::logic_error( "cpu_vector_operations::assign_skip_slices: can only be applied to "
                                    "vectors of sizes x.size<=y.size" );
        }
        size_t index_y = 0;
        for ( size_t j = 0; j < sz_l; j++ )
        {

...
public:
    ...
    {
        decltype(auto) host_view = vec_ops->view(vec);
    ...
    void write_2_vectors_by_side(
        ...
        char sep = ' ') const
    {
        decltype(auto) host_view1 = vec_ops->view(vec1);
        const scalar_type* vec1_data = detail::view_data(host_view1);
        std::vector<scalar_type> vec1_host(vec1_data, vec1_data + sz);

        decltype(auto) host_view2 = vec_ops->view(vec2);

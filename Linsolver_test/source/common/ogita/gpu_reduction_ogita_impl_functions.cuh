...
// service function
template <class T> __device__ __forceinline__ T _shfl(T a, const int j) {
...
// -1 = 0xFFFFFFFF assumes that threadsize = 32, see
...
template <> __device__ __forceinline__ double shuffle(double a, const int j) {
  ...
  return __hiloint2double(_shfl<int>(__double2hiint(a), j),
                          _shfl<int>(__double2loint(a), j));
}

// complex<float> specialization
template <>
__device__ __forceinline__ thrust::complex<float>
shuffle(thrust::complex<float> a, const int j) {
  return thrust::complex<float>(shuffle<float>(a.real(), j),
                                shuffle<float>(a.imag(), j));
}
// complex<double> specialization
template <>
__device__ __forceinline__ thrust::complex<double>
shuffle(thrust::complex<double> a, const int j) {
  return thrust::complex<double>(shuffle<double>(a.real(), j),
                                 shuffle<double>(a.imag(), j));
}

#endif

// FMA specilization
template <class T> __device__ __forceinline__ T _fma(T a, T b, T c) = delete;

template <> __device__ __forceinline__ float _fma(float a, float b, float c) {
...
template <class T>
__device__ __forceinline__ T __GPU_REDUCTION_OGITA_H__two_sum_device(T &t, T a,
                                                                     T b) {
  T s = a + b;
  T bs = s - a;
  T as = s - bs;
...
template <>
__device__ __forceinline__ thrust::complex<float>
__GPU_REDUCTION_OGITA_H__two_prod_device(thrust::complex<float> &t,
...
template <>
__device__ __forceinline__ thrust::complex<double>
__GPU_REDUCTION_OGITA_H__two_prod_device(thrust::complex<double> &t,
...
__device__ __forceinline__
    typename gpu_reduction_ogita_type::type_complex_cast<T>::T
    cuda_abs(T val) {
  return (abs(val));
}
template <> __device__ __forceinline__ double cuda_abs(double val) {
  return (fabs(val));
}
template <> __device__ __forceinline__ float cuda_abs(float val) {
  return (fabsf(val));
}

template <class T>
__device__ __forceinline__ T __GPU_REDUCTION_OGITA_H__two_asum_device(T &t, T a,
                                                                      T b) {
  const T abs_b = cuda_abs<T>(b);
  T s = a + abs_b;
  T bs = s - a;
  T as = s - bs;
  t = (abs_b - bs) + (a - as);

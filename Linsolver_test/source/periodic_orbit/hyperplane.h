...
public:
    ...
    {
        //xk_val = - n_reduced'*(vec)./section.n(section.index, 1);
        T projection = -vec_ops_->scalar_prod(n0_reduced_, const_cast<T_vec&>(x1_p) )/(vec_ops_->get_value_at_point(index_, n_) );
        vec_ops_->set_value_at_point(projection, index_, x1_p);
    }
...
private:
    ...
    //this logic problems with vector operations. It is remed for now

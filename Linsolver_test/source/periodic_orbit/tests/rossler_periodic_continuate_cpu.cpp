...
//   for continuation
    ...
    real a_param = 0.1, b_param = 0.1, c_param = 4.0, max_time_simlation = 100.0, max_param_val = 0.4; 
    int which_param = 0;
    ...
    periodic_orbit_nonlinear_operator_t periodic_orbit_nonlin_op(&vec_ops, &rossler, &log_time, 300.0, param, scheme_name);        
    ...
    // continuate
    knots_t knots;
    knots.add_element({0.05, 0.1, 0.4});
    ...
    vec_t z, x_0s, x_1s, f, f_lambda, d_x, x1_p;
    vec_ops.init_vectors(z, x_0s, x_1s,f_lambda, f, d_x, x1_p); vec_ops.start_use_vectors(z, x_0s, x_1s,f_lambda, f, d_x, x1_p);
    real d_lambda = 0.0;
    ...
    periodic_orbit_nonlin_op.F_and_jacobian_alpha(f, f_lambda);
    log_newton.info_f("x0 = (%le,%le,%le), lambda_0 = %le, f = (%le,%le,%le), f_lambda = (%le,%le,%le)", x0[0], x0[1], x0[2], lambda_0, f[0], f[1], f[2], f_lambda[0], f_lambda[1], f_lambda[2] );    
    ...
    {        
        ...
        while(newton_norm > 1.0e-8)
        {
            ...
            log_newton.info_f("%lu: newton_norm update: %le -> %le, %le -> %le", iters, newton_norm_prev, newton_norm, lambda_1_priv, lambda_1);
    ...
    // continuate.set_steps(max_S, ds_0, ds_max);
    ...
    // bif_diag_curve_t* bdf = new bif_diag_curve_t();
    // log_newton.info("init_new_curve()");
    // bif_diag.init_new_curve();
    // log_newton.info("get_current_ref( bdf ):");
    // bif_diag.get_current_ref( bdf );
    // log_newton.info("continuate_curve( bdf , x0, mu )");
    // continuate.continuate_curve( bdf , x0, mu );
    // log_newton.info("close_curve()");
    // bif_diag.close_curve();
    // delete bdf;

    vec_ops.stop_use_vectors(z, x_0s, x_1s, f_lambda, d_x, x1_p); vec_ops.free_vectors(z, x_0s, x_1s, f_lambda, d_x, x1_p);

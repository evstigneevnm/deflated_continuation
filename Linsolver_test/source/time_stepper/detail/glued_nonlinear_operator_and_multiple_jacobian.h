#ifndef __TIME_STEPPER_LYAPUNOV_EXPONENTS_GLUED_NONLINEAR_OPERATOR_AND_JACOBIAN_H__
#define __TIME_STEPPER_LYAPUNOV_EXPONENTS_GLUED_NONLINEAR_OPERATOR_AND_JACOBIAN_H__
...
template<class VectorOperations, class NonlinearOperator, std::size_t GluedVectors>
class glued_nonlinear_operator_and_multiple_jacobian
{
private:
    struct fake_deleter
    {
...
public:
    using glued_vector_operations_type = scfd::linspace::glued_vector_space<VectorOperations, GluedVectors>;
...
public:
    ...
    glued_nonlinear_operator_and_multiple_jacobian(VectorOperations* vec_ops_p, NonlinearOperator* nonlin_op_p):
    vec_ops_(vec_ops_p),
    nonlin_op_(nonlin_op_p)
    {

        std::shared_ptr<VectorOperations> aaa(vec_ops_, fake_deleter() );
        glued_vector_operations_ = new glued_vector_operations_type(aaa);
    }
    ~glued_nonlinear_operator_and_multiple_jacobian()
    {
    ...
    void F(const T time_p, const T_vec& in_p, const T param_p, T_vec& out_p)
    {
        ...
        {
            nonlin_op_->jacobian_u( in_p.comp(l), out_p.comp(l) );
        }
    }

...
public:
    ...
    sol_storage(sol_storage_)
    {
        ...
        vec_ops->init_vector(x_fixed); vec_ops->start_use_vector(x_fixed);
    }
    ~system_operator_deflation_with_translation()
    {
        ...
        vec_ops->stop_use_vector(x_fixed); vec_ops->free_vector(x_fixed);
    }

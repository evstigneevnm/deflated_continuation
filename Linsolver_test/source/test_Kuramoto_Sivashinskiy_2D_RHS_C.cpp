...
int main(int argc, char const *argv[])
{
    ...
    gpu_vector_operations_real_reduced *vec_ops_C_im = new gpu_vector_operations_real_reduced(Nx*My-1, CUBLAS);
    
    KS_2D *KS2D = new KS_2D(2.0, 4.0, Nx, Ny, vec_ops_R, vec_ops_C, vec_ops_C_im, CUFFT_C2R);
    ...
    //test nonlinear part
    KS2D->F((const complex*&)u_in_hat, 1.0, (complex*&)u_out_hat);

    //test linear part
    KS2D->set_linearization_point(u_out_hat, 1.0);  //set linearization
    KS2D->jacobian_u((const complex*&)du_in_hat, (complex*&)du_out_hat); //apply J_u

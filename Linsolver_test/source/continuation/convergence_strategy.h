...
public:    
    ...
    void set_convergence_constants(T tolerance_, unsigned int maximum_iterations_, T relax_tolerance_factor_, int relax_tolerance_steps_, T newton_wight_ = T(1), bool store_norms_history_ = false, bool verbose_ = true, unsigned int stagnation_max_ = 10)
    {
        ...
        log->info_f("continuation::convergence: check: relax_tolerance_factor = %le, maximum_norm_increase = %le.", (double)relax_tolerance_factor, (double)maximum_norm_increase_);        
    ...
    bool check_convergence(nonlinear_operator* nonlin_op, T_vec& x, T& lambda, T_vec& delta_x, T& delta_lambda, int& result_status)
    {
        ...
        log->info_f("continuation::convergence: iteration: %i, max_iterations: %i, residuals n: %le, n+1: %le, min_value: %le, result_status: %i => %s, is_finished = %s, newton_wight = %le.",iterations, maximum_iterations, (double)normFx, (double)normFx1, double(min_value), result_status,  result_status_string.c_str(), finish_string.c_str(), newton_wight );

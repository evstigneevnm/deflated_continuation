...
#include <numerical_algos/lin_solvers/bicgstab_real_resid.h>
#include <numerical_algos/lin_solvers/bicgstabl_real_resid.h>
...
{
    if (argc < 12) {
        std::cout << "USAGE: " << std::string(args[0]) << " <mesh_sz> <a> <re> <max_iters> <rel_tol> <use_precond_resid> <use_real_resid> <basis_sz> <lin_solver_type> <result_fn> <convergency_fn>"  << std::endl;
    ...
    bool                    use_real_resid = atoi(args[7]);
    ...
    lin_solver_bicgstab.set_use_real_resid(use_real_resid);
    lin_solver_bicgstab.set_use_precond_resid(use_precond_resid);
    lin_solver_bicgstabl.set_use_real_resid(use_real_resid);
